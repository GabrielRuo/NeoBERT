#!/bin/bash
#SBATCH --job-name=bert-ablations-05.2
...
cmd=(
    accelerate launch \
    --config_file=$HOME/neo-bert/conf/accelerate_ddp.yaml \
    ...
    datacollator=mlm_15 \
    optimizer=soap \
    optimizer.hparams.lr=3e-4 \
    scheduler=cosine_decay \
    trainer.gradient_accumulation_steps=1 \
    dataloader.train.batch_size=128 \
    dataset.path_to_disk=tokenized_datasets/falcon-refinedweb_google_512 \
)
