...
def train_and_eval_sweep(cfg: DictConfig):
    ...
    if cfg.dataset.name == "crammingpile": #careful to have correct configs in pretraining.yaml cfg such that cfg.dataloader is CRAMMINGdataloader,  cfg.tokenizer is CRAMMINGtokenizer cfg.datacollator is CRAMMINGmlm_15
        ...
        test_dataloader = get_dataloader(test_dataset, tokenizer, dtype=dtype_pad_mask,**cfg.dataloader.test,**cfg.datacollator) #ultimately should be moved to cfg.dataloader.test
        ...
        for batch in dataloader:
            ...
            if metrics["train/batches"] % cfg.trainer.gradient_accumulation_steps != 0: # ATTENTION IN OUR CURRENT SETTINGS WE DO NOT USE GRADIENT ACCUMULATION HENCE THIS IS NEVER ENTERED
                ...
                if cfg.model.type == "mop":
                    ...
                    #run the analyses
                    analysistraining(batch,model_output,metrics["train/steps"],metrics) #log correlation between mse and expert usage per sequence
    ...
    model.eval()
    analysetrained = AnalysisTrainedModel(cfg,accelerator,cfg.test.max_steps,cfg.test.buffer_size_seq,cfg.test.buffer_size_token)
    test_metrics = {}
    step = 0
    # Use skipped_train_dataloader the first epoch after resuming
    dataloader = test_dataloader if skipped_train_dataloader is None else skipped_train_dataloader
    ...
    for batch in dataloader:
        ...
        if batch["input_ids"].shape[0] != cfg.dataloader.test.batch_size:
            batch, stored_batch = to_target_batch_size(batch, stored_batch, cfg.dataloader.test.batch_size)
