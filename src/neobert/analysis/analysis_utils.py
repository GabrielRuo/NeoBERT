...
class AnalysisMetrics:
    ...
    def get_entropy(self, gate_logits: Union[torch.Tensor, tuple[torch.Tensor], None], attention_mask: torch.Tensor,cfg: DictConfig, entropy_loss_epsilon: float = 1e-5) -> torch.Tensor:
        ...
        concatenated_gate_weights = torch.nn.functional.softmax(concatenated_gate_logits, dim=-1) #shape (n_layers,batch_size, seq_length,num_experts)
        concatenated_gate_weights = concatenated_gate_weights.reshape(n_layers,-1,num_experts)#shape (n_layers,batch_size*seq_length,num_experts)
        batch_weights = torch.sum(concatenated_gate_weights, dim=1) #shape (n_layers,num_experts)
        batch_probs = batch_weights / torch.sum(batch_weights, dim=-1, keepdim=True) #shape (n_layers,num_experts)

        # Compute the entropy of the batch at each layer
        batch_entropy = -torch.sum(batch_probs * torch.log(batch_probs + entropy_loss_epsilon), dim=-1) #shape (n_layers)
    ...
    #layer level metrics
    def get_difference_mean_expert_usage_across_layers(self,mean_expert_usage_per_layer: torch.Tensor) -> torch.Tensor:
        ...
        if len(differences) > 0:
            ...
            mean_difference = torch.tensor(0.0, device=mean_expert_usage_per_layer.device)
    ...
    def get_entropy_per_sequence(self, gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig, entropy_loss_epsilon: float = 1e-5) -> torch.Tensor:
        ...
        concatenated_gate_weights = torch.nn.functional.softmax(concatenated_gate_logits, dim=-1) #shape (n_layers,batch_size, seq_length,num_experts)
        concatenated_gate_weights = concatenated_gate_weights.reshape(-1,seq_length,num_experts)#shape (n_layers*batch_size, seq_length,num_experts)
        sequence_weights = torch.sum(concatenated_gate_weights, dim=1) #shape (n_layers*batch_size, num_experts)
        sequence_probs = sequence_weights / torch.sum(sequence_weights, dim=-1, keepdim=True) #shape (n_layers*batch_size, num_experts)

        # Compute the entropy of the distibution of each sequence at each layer
        entropy_per_sequence = -torch.sum(sequence_probs * torch.log(sequence_probs + entropy_loss_epsilon), dim=-1) #shape (n_layers*batch_size)
    ...
    def get_inter_sequence_normalised_expert_cost_rel_std(self, router_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig) -> torch.Tensor:
        """_summary_
        ...
        Returns: 
        """
        normalised_mean_expert_usage_loss_per_sequence = self.get_normalised_expert_usage_cost_per_sequence(router_logits, attention_mask, cfg) #shape (batch_size,)
    ...
    def get_intra_sequence_normalised_expert_cost_rel_std(self, router_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig) -> torch.Tensor:
        """_summary_
    ...
    def get_normalised_expert_usage_cost_per_masked_token(self,router_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, batch:dict, cfg: DictConfig,tokenizer) -> torch.Tensor:
        """_summary_
        ...
        mask = ((batch["input_ids"] == mask_token_id) & (batch["input_ids"] != padding_token_id)).float()  # (batch_size, seq_len) # keep masked tokens which are not padding tokens
    ...
    def _get_normalised_expert_usage_per_token_and_layer(self, router_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig) -> tuple[torch.Tensor,torch.Tensor]:
