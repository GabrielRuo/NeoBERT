...
class AnalysisLogger(AnalysisMetrics):
    def __init__(self,cfg,accelerator,buffer_size_finetuning_token=5000,buffer_size_finetuning_seq=500,buffer_size_seq=1000,buffer_size_token=5000):
        self.cfg = cfg
    ...
    def log_different_expert_usage_consecutive_layers(self, gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig,metrics_dict:defaultdict):
        #metrics
        mean_expert_usage_per_layer_per_token, _ = self._get_mean_and_max_expert_usage_per_layer_per_token(gate_logits, attention_mask, cfg)
    ...
    def log_correlation_between_ce_and_expert_usage_token(self, metrics_dict:defaultdict,step,prefix="train"):
        ...
        if len(self.expert_usage_token_buffer) >= self.buffer_size_token and len(self.ce_loss_token_buffer) >= self.buffer_size_token:
            ...
            self._visualise_correlation(expert_usage_list, cross_entropy_loss_list,metrics_dict,x_axis_name="Cross Entropy Loss per Token", y_axis_name="Expert Usage per Token", title="Expert Usage vs Cross Entropy Loss", step=step,prefix=prefix)
    ...
    def _visualise_correlation(self,expert_usage_list, task_loss_list,metrics_dict, x_axis_name="Cross Entropy Loss per Token", y_axis_name="Expert Usage per Token", title = "Expert Usage vs Task Loss", step=None, prefix="train"):
                ...
                metrics_dict[f"{prefix}/{title}_fixed_bounds"] = wandb.Image(scatter_plot_filename_fixed)
    ...
    def accumulate_task_loss_and_expert_usage_seq(self,logits: torch.Tensor,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig,batch: dict,is_regression, num_labels:int):
        
        normalised_expert_usage_cost_per_seq = self.get_normalised_expert_usage_cost_per_sequence(gate_logits, attention_mask, cfg)# shape (batch_size,)
        task_loss_per_seq = self.get_task_loss_per_sequence(logits,batch,is_regression, num_labels)# batch size
        ...
        # Accumulate in buffers
        self.expert_usage_seq_buffer_finetuning.extend(normalised_expert_usage_cost_per_seq.detach().cpu().tolist())
        self.task_loss_seq_buffer_finetuning.extend(task_loss_per_seq.detach().cpu().tolist())
        # print(len(self.expert_usage_seq_buffer_finetuning))
        # print(len(self.task_loss_seq_buffer_finetuning))

    def accumulate_task_loss_and_expert_usage_token(self,logits: torch.Tensor,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig,batch: dict,is_regression, num_labels:int):
        
        normalised_expert_usage_cost_per_token = self.get_normalised_expert_usage_cost_per_token(gate_logits, attention_mask, batch,cfg,self.tokenizer)# shape (batch_size,seq_length)
        task_loss_per_token = self.get_task_loss_per_token(logits,batch,is_regression, num_labels,attention_mask)# batch size*seq_length
        task_loss_per_token = task_loss_per_token.reshape(-1)  # Flatten to (num_valid_tokens_in_batch,)
        
        #filter out the padding tokens
        attention_mask_flat = attention_mask.reshape(-1)  # Flatten to (batch_size * seq_length,)
        task_loss_per_token = task_loss_per_token[attention_mask_flat == 0]
        normalised_expert_usage_cost_per_token = normalised_expert_usage_cost_per_token[attention_mask_flat == 0]
        ...
        # Accumulate in buffers
        self.expert_usage_token_buffer_finetuning.extend(normalised_expert_usage_cost_per_token.detach().cpu().tolist())
        self.task_loss_token_buffer_finetuning.extend(task_loss_per_token.detach().cpu().tolist())
    ...
    def log_correlation_between_task_loss_and_expert_usage_seq(self,metrics_dict:defaultdict,step,prefix="train"):
        
        # When buffer is full, compute correlation and log, then reset
        if len(self.expert_usage_seq_buffer_finetuning) >= self.buffer_size_finetuning_seq and len(self.task_loss_seq_buffer_finetuning) >= self.buffer_size_finetuning_seq:
            # Truncate to buffer_size in case of overflow

            expert_usage_list = self.expert_usage_seq_buffer_finetuning[:self.buffer_size_finetuning_seq]
            task_loss_list = self.task_loss_seq_buffer_finetuning[:self.buffer_size_finetuning_seq]

            self._visualise_correlation(expert_usage_list, task_loss_list, metrics_dict = metrics_dict, x_axis_name="Expert Usage per seq", y_axis_name="Task Loss per seq", title = "Correlation between Expert Usage and Task Loss per seq", step=step, prefix=prefix)


            expert_usage_arr = torch.tensor(self.expert_usage_seq_buffer_finetuning[:self.buffer_size_finetuning_seq])
            task_loss_arr = torch.tensor(self.task_loss_seq_buffer_finetuning[:self.buffer_size_finetuning_seq])
            # Compute Pearson correlations
            # metrics_dict[f"{prefix}/expert_usage_per_seq"] = expert_usage_arr
            # metrics_dict[f"{prefix}/cross_entropy_per_seq"] = cross_entropy_loss_arr
            if expert_usage_arr.std() > 0 and task_loss_arr.std() > 0:
                correlation = torch.corrcoef(torch.stack([expert_usage_arr, task_loss_arr]))[0, 1].item()
            else:
                correlation = 0.0
    ...
    def log_correlation_between_task_loss_and_expert_usage_token(self,metrics_dict:defaultdict,step,prefix="train"):
        
        # When buffer is full, compute correlation and log, then reset
        if len(self.expert_usage_token_buffer_finetuning) >= self.buffer_size_finetuning_token and len(self.task_loss_token_buffer_finetuning) >= self.buffer_size_finetuning_token:
            # Truncate to buffer_size in case of overflow

            expert_usage_list = self.expert_usage_token_buffer_finetuning[:self.buffer_size_finetuning_token]
            task_loss_list = self.task_loss_token_buffer_finetuning[:self.buffer_size_finetuning_token]


            self._visualise_correlation(expert_usage_list, task_loss_list, metrics_dict,x_axis_name="Expert Usage per Token",y_axis_name="Task Loss per Token", title = "Correlation between Expert Usage and Task Loss per Token", step=step, prefix=prefix)


            expert_usage_arr = torch.tensor(self.expert_usage_token_buffer_finetuning[:self.buffer_size_finetuning_token])
            task_loss_arr = torch.tensor(self.task_loss_token_buffer_finetuning[:self.buffer_size_finetuning_token])
            # Compute Pearson correlations
            # metrics_dict[f"{prefix}/expert_usage_per_seq"] = expert_usage_arr
            # metrics_dict[f"{prefix}/cross_entropy_per_seq"] = cross_entropy_loss_arr
            if expert_usage_arr.std() > 0 and task_loss_arr.std() > 0:
            ...
            # Log correlation
            metrics_dict[f"{prefix}/expert_usage_task_loss_per_token_corr"] = correlation
            # self.accelerator.log({f"{prefix}/expert_usage_task_loss_per_seq_corr": correlation})
            # Reset buffers
            self.expert_usage_token_buffer_finetuning = []
            self.task_loss_token_buffer_finetuning = []
    ...
    def log_std_rel_metrics(self,logits: torch.Tensor,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig,metrics_dict:defaultdict):
        ...
        self.inter_token_normalised_expert_cost_rel_std, self.inter_token_normalised_expert_cost_std, self.inter_token_normalised_expert_cost_mean = self.get_inter_token_normalised_expert_cost_rel_std(gate_logits, attention_mask, cfg)
        ...
        metrics_dict["train/inter_token_normalised_expert_cost_std"]= self.inter_token_normalised_expert_cost_std.item()
        metrics_dict["train/inter_token_normalised_expert_cost_mean"]= self.inter_token_normalised_expert_cost_mean.item()
    ...
    def _visualise_individual_token_path(self,gate_logits, token_index, token_str, visualisations_list, sequence_str=None):
            # Stack logits if tuple
            if isinstance(gate_logits, tuple):
                compute_device = gate_logits[0].device
                target_stacked_gate_logits = torch.stack([layer_gate.to(compute_device) for layer_gate in gate_logits], dim=0)  # n_layers, batch_size*seq_thth, n_experts
            else:
    ...
    def visualise_token_routing_paths(self, batch, gate_logits, metrics_dict,num_visualisations=50):
        ...
        while len(self.visualisations_list) < num_visualisations:
            ...
            self._visualise_individual_token_path(gate_logits, seq_index * seq_length + token_index, token_str, self.visualisations_list, sequence_str=sequence_str)
...
class AnalysisTraining(AnalysisLogger):
    ...
    def __call__(self,batch,model_output,step,metrics_dict):
        ...
        if step % self.cfg.wandb.log_interval == 0:
            ...
            self.accumulate_ce_and_expert_usage_token(self.logits,self.gate_logits, self.attention_mask, self.cfg,self.batch,self.tokenizer)
            self.log_correlation_between_ce_and_expert_usage_seq(metrics_dict)
            self.log_correlation_between_ce_and_expert_usage_token(metrics_dict,step=metrics_dict["train/steps"])
...
class AnalysisTrainedModel(AnalysisLogger):
    ...
    def __call__(self,batch,model_output,step,metrics_dict):
        ...
        if step == self.max_steps - 1:
            ...
            self.log_correlation_between_ce_and_expert_usage_token(metrics_dict,step=metrics_dict["train/steps"],prefix="test")
...
class AnalysisFinetuning(AnalysisLogger):
    ...
    def __call__(self,batch,model_output,eval_step,training_step,metrics_dict,is_regression, num_labels:int,eval_dataloader,training:bool):
        ...
        if not self.training:
            ...
            self.buffer_size_finetuning_token = len(eval_dataloader.dataset)*10 #assuming average seq length >10 tokens. We don't need it to be exact here
        ...
        if self.training:
            ...
            if self.training_step % self.cfg.wandb.log_interval == 0:
                ...
                self.log_different_expert_usage_consecutive_layers(self.gate_logits, self.attention_mask, self.cfg, training_metrics_dict)   
                
                self.log_entropy_metrics(self.gate_logits, self.attention_mask, self.cfg, training_metrics_dict)
                self.log_std_rel_metrics(self.logits,self.gate_logits, self.attention_mask, self.cfg, training_metrics_dict)
        ...
        #accumulate the task loss and expert usage for correlation over the whole eval set
            self.accumulate_task_loss_and_expert_usage_seq(self.logits,self.gate_logits, self.attention_mask, self.cfg,self.batch,is_regression, num_labels)
            self.accumulate_task_loss_and_expert_usage_token(self.logits,self.gate_logits, self.attention_mask, self.cfg,self.batch,is_regression, num_labels)

            # self.accumulate_ce_and_expert_usage_token(self.logits,self.gate_logits, self.attention_mask, self.cfg,self.batch,self.tokenizer)

            if self.eval_step == len(eval_dataloader)-1:
                self.log_correlation_between_task_loss_and_expert_usage_seq(eval_metrics_dict,step=self.training_step)
                self.log_correlation_between_task_loss_and_expert_usage_token(eval_metrics_dict,step=self.training_step)
...
class AnalysisTestTrainedModel(AnalysisLogger):
    ...
    def __call__(self,batch,model_output,step,metrics_dict):
        ...
        self.accumulate_ce_and_expert_usage_token(self.logits,self.gate_logits, self.attention_mask, self.cfg,self.batch,self.tokenizer)
        self.log_correlation_between_ce_and_expert_usage_seq(metrics_dict)
        self.log_correlation_between_ce_and_expert_usage_token(metrics_dict,step=self.step)
