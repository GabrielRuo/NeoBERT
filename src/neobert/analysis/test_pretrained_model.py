...
def pretrained_model_tester(cfg_test):
    ...
    accelerator.init_trackers(
        ...
        init_kwargs={
            "wandb": {
                ...
                "config": OmegaConf.to_container(cfg_test) | {"distributed_type": accelerator.distributed_type},
    ...
    if "glue" in cfg_test.saved_model.checkpoint:
        ...
        state_dict_path = os.path.join(cfg_test.saved_model.base_path, cfg_test.saved_model.checkpoint,"state_dict.pt")
    else:
        state_dict_path = os.path.join(cfg_test.saved_model.base_path, "model_checkpoints", cfg_test.saved_model.checkpoint,"state_dict.pt")
    ...
    test_dataset = get_dataset(cfg, 
            **cfg_test.dataset.test)

    test_dataloader = get_dataloader(
        test_dataset, tokenizer, dtype=dtype_pad_mask,
        **cfg_test.dataloader.test, **cfg_test.datacollator
    )
    ...
    if not has_pretrained_lm_head:
        max_steps = 10000
        print(f"The pretrained model does not have a pretrained LM head. We will finetune the LM head on the train dataset for  {max_steps} steps.")
        train_dataset = get_dataset(cfg, 
            **cfg.dataset.train)

        train_dataloader = get_dataloader(
            train_dataset, tokenizer, dtype=dtype_pad_mask,
            **cfg.dataloader.train, **cfg.datacollator
        )
        optimizer = get_optimizer(neobert_model, accelerator.distributed_type, name=cfg.optimizer.name, **cfg.optimizer.hparams)
        ...
        test_dataloader,train_dataloader, neobert_model,optimizer,scheduler = accelerator.prepare(
        ...
        max_batches = None  # keep behavior consistent; change if you want to limit batches
        ...
        for i, batch in enumerate(pbar_train):
            ...
            #target
            model_output = neobert_model(batch["input_ids"], batch.get("attention_mask", None), output_expert_usage_loss=True, output_router_logits=True)
            logits = model_output["logits"]

            num_masked_tokens = cfg.dataloader.train.batch_size * cfg.tokenizer.max_length * i *cfg.datacollator.mlm_probability # does not actually matter since we use alpha_scaling = 0
            ...
            if cfg.trainer.gradient_clipping is not None and cfg.trainer.gradient_clipping > 0:
                    accelerator.clip_grad_norm_(neobert_model.parameters(), cfg.trainer.gradient_clipping)
            ...
            metrics["ft/local_num_correct"] += (logits.argmax(dim=-1) == batch["labels"]).sum().item()

            metrics["ft/local_num_pred"] += (batch["labels"] != -100).sum().item()

            if metrics["ft/local_num_pred"] > 0:
                metrics["ft/accuracy"] = metrics["ft/local_num_correct"] / metrics["ft/local_num_pred"]
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(pbar_test):
            ...
            if batch["input_ids"].shape[0] != cfg_test.dataloader.test.batch_size:
                batch, stored_batch = to_target_batch_size(batch, stored_batch, cfg_test.dataloader.test.batch_size)
            ...
            metrics["test/local_num_correct"] += (logits.argmax(dim=-1) == batch["labels"]).sum().item()

            metrics["test/local_num_pred"] += (batch["labels"] != -100).sum().item()

            if metrics["test/local_num_pred"] > 0:
                metrics["test/accuracy"] = metrics["test/local_num_correct"] / metrics["test/local_num_pred"]
