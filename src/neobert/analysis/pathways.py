...
def get_mean_max_weight_per_layer_vector(gate_logits_tensor):
    ...
    expert_indices = torch.arange(
        n_experts, device=gate_logits_tensor.device
    ).float()  # Shape: (n_experts,)
    mean_expert_index_vector = torch.einsum(
        "ijk,k->ij", gate_logits_tensor, expert_indices
    )  # n_layers, batch_size*n_seq
    max_expert_index_vector = torch.argmax(
        gate_logits_tensor, dim=-1
    )  # n_layers, batch_size*n_seq
...
def pathways_analysis(cfg_predictor):
    ...
    state_dict_path = os.path.join(
        ...
        "model_checkpoints",
        cfg_predictor.saved_model.checkpoint,
        "state_dict.pt",
    )
...
def _visualise_token_path(
    target_gate_logits, token_index, token_str, visualisations_list
):
    ...
    if isinstance(target_gate_logits, tuple):
        ...
        target_stacked_gate_logits = torch.stack(
            [layer_gate.to(compute_device) for layer_gate in target_gate_logits], dim=0
        )  # n_layers, batch_size*seq_length, n_experts
    else:
...
def run_test_batches(
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_dataloader):
            ...
            mean_expert_index_vector, max_expert_index_vector = (
                get_mean_max_weight_per_layer_vector(routing_weights)
            )

            # mean_expert_index_vector : shape (n_layers, tokens)
            # transpose to tokens x n_layers and add to accumulators
            mean_tokens_by_layers = (
                mean_expert_index_vector.T.cpu().numpy()
            )  # shape (tokens, n_layers)
            max_tokens_by_layers = (
                max_expert_index_vector.T.cpu().numpy()
            )  # shape (tokens, n_layers)
            ...
            try:
                ...
                full_tokens = (
                    rw.permute(1, 0, 2).reshape(tokens_n, n_layers * n_experts).numpy()
                )
            ...
            if len(mean_accum) >= n_agg_batches:
                mean_agg = np.concatenate(
                    mean_accum, axis=0
                )  # (n_agg_batches * tokens, n_layers)
                ...
                if accelerator.is_main_process:
                    ...
                    try:
                        ...
                        if n_components_mean < 1:
                            raise ValueError(
                                f"Not enough samples/features for PCA on mean_agg: shape={mean_agg.shape}"
                            )
                        ...
                        plt.ylabel(
                            "Principal Component 2" if n_components_mean > 1 else ""
                        )
                        ...
                        if n_components_max < 1:
                            raise ValueError(
                                f"Not enough samples/features for PCA on max_agg: shape={max_agg.shape}"
                            )
                        ...
                        plt.xlabel("Principal Component 1")
                        plt.ylabel(
                            "Principal Component 2" if n_components_max > 1 else ""
                        )
                        ...
                        try:
                            if len(full_accum) > 0:
                                full_agg = np.concatenate(
                                    full_accum, axis=0
                                )  # (n_samples, n_layers * n_experts)
                                n_samples_full, n_features_full = full_agg.shape
                                n_components_full = min(
                                    2, n_features_full, n_samples_full
                                )
                                if n_components_full >= 1:
                                    scaler_full = StandardScaler()
                                    full_agg_scaled = scaler_full.fit_transform(
                                        full_agg
                                    )
                                    ...
                                    plt.scatter(
                                        ...
                                        (
                                            full_2d[:, 1]
                                            if n_components_full > 1
                                            else np.zeros_like(full_2d[:, 0])
                                        ),
                                        alpha=0.5,
                                    )
                                    plt.title(
                                        "PCA of Full Routing-Weight Vectors (aggregated)"
                                    )
                                    plt.xlabel("Principal Component 1")
                                    plt.ylabel(
                                    ...
                                    metrics_dict["test/full_expert_weights_pca"] = (
                                        wandb.Image(full_fig)
                                    )
                        except Exception as e:
                            print(
                                f"[Warning] PCA on full routing-weight vectors skipped due to error: {e}"
                            )

                        # Log to wandb if available
                        metrics_dict["test/mean_expert_index_pca"] = wandb.Image(
                            mean_fig
                        )
