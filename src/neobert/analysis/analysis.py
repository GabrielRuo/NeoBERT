...
class AnalysisLogger(AnalysisMetrics):
    ...
    def log_and_append_layer_level_metrics(self,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig,step:int):
        """
    ...
    def log_correlation_between_ce_and_expert_usage_token(self,logits: torch.Tensor,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig,batch: dict,tokenizer):
        ...
        if len(self.expert_usage_token_buffer) >= self.buffer_size_token and len(self.ce_loss_token_buffer) >= self.buffer_size_token:
            ...
            if expert_usage_arr.std() > 0 and cross_entropy_loss_arr.std() > 0:
                ...
                correlation = 0.0
            # Log correlation
            self.accelerator.log({"train/expert_usage_cross_entropy_per_token_corr": correlation})
            # Reset buffers
            self.expert_usage_token_buffer = []
            self.ce_loss_token_buffer = []
    ...
    def log_correlation_between_ce_and_expert_usage_seq(self,logits: torch.Tensor,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig,batch: dict):
        ...
        if len(self.expert_usage_seq_buffer) >= self.buffer_size_seq and len(self.ce_loss_seq_buffer) >= self.buffer_size_seq:
            ...
            if expert_usage_arr.std() > 0 and cross_entropy_loss_arr.std() > 0:
                ...
                correlation = 0.0
            # Log correlation
            self.accelerator.log({"train/expert_usage_cross_entropy_per_seq_corr": correlation})
            # Reset buffers
            self.expert_usage_seq_buffer = []
            self.ce_loss_seq_buffer = []

    def log_entropy_metrics(self,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig):
    ...
    def log_std_rel_metrics(self,logits: torch.Tensor,gate_logits: tuple[torch.Tensor], attention_mask: torch.Tensor, cfg: DictConfig):
        #metrics
    ...
    def log_token_expert_usage_df(self,token_expert_usage_dict: defaultdict):
        ...
        self.accelerator.log({
            "token_expert_usage": self.token_expert_usage_table
        })
...
class AnalysisTraining(AnalysisLogger):
    ...
    def __call__(self,batch,model_output,step):
        ...
        if step % self.cfg.wandb.log_interval == 0:
            self.log_and_append_layer_level_metrics(self.gate_logits, self.attention_mask, self.cfg,self.step)
            ...
            self.log_std_rel_metrics(self.logits,self.gate_logits, self.attention_mask, self.cfg)
