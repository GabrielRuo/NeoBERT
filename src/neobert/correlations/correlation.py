...
def compute_correlations(base_path, checkpoint):
    ...
    for batch in dataloader:
        ...
        expert_usages.extend(
            normalised_expert_usage_cost_per_seq.detach().cpu().numpy().flatten()
        )
    ...
    print(
        f"Correlation between MSE loss per sequence and expert usage cost per sequence: {correlation}"
    )


def compute_correlations_across_list_of_models(
    list_of_base_paths=[
    ...
    for base_path in list_of_base_paths:
        print(
            f"Computing correlations for model at {base_path} with checkpoint {checkpoint}"
        )
        cfg = get_config(base_path)
        print(
            f"alpha_start={cfg.model.loss.cost_based_loss_alpha_start},alpha_end={cfg.model.loss.cost_based_loss_alpha_end},schedule_tokens={cfg.model.loss.cost_based_loss_schedule_tokens}"
        )
