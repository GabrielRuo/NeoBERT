...
def trainer(cfg: DictConfig):
    ...
    if accelerator.is_main_process:
        if os.path.isdir(model_dir):
            for file in os.listdir(model_dir):
                if os.path.isfile(os.path.join(model_dir, str(file))):
                    os.remove(os.path.join(model_dir, str(file)))
        os.makedirs(model_dir, exist_ok=True)
    accelerator.wait_for_everyone()
    ...
    #  - If loading from the Hub, AutoModelForSequenceClassification provides a ready-made classification head.
    #  - For some models (e.g. masked-LM variants) one could load the base LM and wrap/replace its head (see commented example).
    #  - If using the local implementation, NeoBERTForSequenceClassification constructs the backbone and adds the classification head in its __init__.
    # When loading pretrained weights later we call load_state_dict(..., strict=False) so classifier/head weights can be (re)initialized if they don't match.
    if cfg.model.from_hub:
    ...
    if not cfg.model.from_hub:
        ...
        #         model, cfg.model.pretrained_checkpoint_dir, tag=str(cfg.model.pretrained_checkpoint)
        #     )
        # except FileNotFoundError:
        #     state_dict = torch.load(
        #         os.path.join(cfg.model.pretrained_checkpoint_dir, str(cfg.model.pretrained_checkpoint), "state_dict.pt")
        #     )
    ...
    for epoch in range(starting_epoch, cfg.trainer.num_train_epochs):
        for batch in train_dataloader:
            ...
            if (
                ...
                print(f"  Global step: {real_sched.last_epoch}")
                if hasattr(real_sched, "_milestones"):
                    print(f"  Milestones: {real_sched._milestones}")
                print(f"  Current LR: {optimizer.param_groups[0]['lr']:.6f}")

                # ---- Decay rate estimation ----
                lrs = [pg["lr"] for pg in optimizer.param_groups]
                print(f"Step {completed_steps} | Current LR {lrs[0]:.8f}")

                if completed_steps > 0:
                    decay_rate = prev_lr - lrs[0]
                    print(f"  Approx instantaneous decay rate: {decay_rate:.8e}")
                ...
                eval_metric = get_evaluation(
                    ...
                    analysistraining=analysistraining,
                    num_labels=num_labels,
                    training_step=completed_steps,
                )["eval_metric"]
                ...
                #         "train_metric": train_metric,
                #         "train_loss": total_loss / completed_steps,
                #         "task_loss": task_loss/ completed_steps,
                #         "expert_loss":expert_loss/ completed_steps,
                #         "load_balancing_loss": load_balancing_loss / completed_steps,
                #         "epoch": epoch,
                #         "step": completed_steps,
                #         "learning_rate": optimizer.param_groups[0]["lr"],
                #         "train/grad_norm": grad_norm_per_param,
...
#
# So, batch["input_ids"].shape == (batch_size, seq_len)

# freqs_cis is typically the rotary positional embedding frequency tensor.
# Its shape is usually (seq_len, rotary_dim), where:
#   - seq_len: sequence length (should match input sequence length)
#   - rotary_dim: the number of rotary dimensions (often half the hidden size per attention head, e.g., head_dim // 2)
#
# So, freqs_cis.shape == (seq_len, rotary_dim)
