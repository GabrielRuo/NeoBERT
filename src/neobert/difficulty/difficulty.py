...
def measure_difficulty(cfg_dif):
    ...
    textattack_model = AutoModelForSequenceClassification.from_pretrained(
    ...
    def collate_fn(batch):
        ...
        return {"sentence1": s1, "sentence2": s2, "labels": labels, "inputs": inputs}
    ...
    neobert_model = NeoBERTLMHead(
        NeoBERTConfig(
            **cfg.model, **cfg.tokenizer, pad_token_id=textattack_tokenizer.pad_token_id
        )
    )  # à voir quel tokenizer on utilise. IL va falloir les regarder de près les deux
    state_dict_path = os.path.join(
        cfg_dif.saved_model.base_path,
        "model_checkpoints",
        cfg_dif.saved_model.checkpoint,
        "state_dict.pt",
    )
        ...
        def forward(
            ...
            **kwargs,
            ...
            return self.backbone(
                ...
                output_expert_usage_loss=output_expert_usage_loss,
            )
    ...
    class NeoBERTForSTSB(nn.Module):
        def __init__(self, hidden_size, dropout_prob=0.1):
            ...
            self.config = (
                neobert_backbone.config
            )  # not quite same as cfg since its a NeoBERTconfig but we could equivalently have used cfg
            self.expert_dims = [
                int(expert_size) for expert_size in self.config.expert_sizes.split(",")
            ]
        ...
        def forward(
            ...
            expert_usage_loss = self._MoEBlockRoutingCost(
            ...
            return {
                "loss": loss,
                "logits": logits,
                "expert_usage_loss": expert_usage_loss,
            }

        def _MoEBlockRoutingCost(
            self,
            ...
            if len(self.expert_dims) > 1:
                ...
                routing_costs = (
                    normalised_expert_sizes**self.config.expert_cost_exponent
                )
                ...
                # sum across layers
                expert_usage_loss = torch.sum(
                    expert_usage_loss, dim=0
                )  # shape (batch_size,seq_length)
                # Average over all tokens in the sequence
                normalised_mean_expert_usage_loss = (
                    torch.sum(expert_usage_loss, dim=1) / number_of_tokens_per_seq
                )  # shape (batch_size,)
            else:
    ...
    neobert_stsb_model = NeoBERTForSTSB(
        hidden_size=cfg.model.hidden_size, dropout_prob=0.1
    )
