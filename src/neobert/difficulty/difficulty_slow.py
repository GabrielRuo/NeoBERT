...
def measure_difficulty(cfg_dif):
    ...
    for i, example in enumerate(val_set):
        ...
        textattack_inputs = textattack_tokenizer(
            example["sentence1"],
            example["sentence2"],
            truncation=True,
            max_length=128,
        ...
        with torch.no_grad():
            textattack_outputs = textattack_model(**textattack_inputs)
            textattack_pred = textattack_outputs.logits.item()
        
        # NeoBERT model evaluation (using same tokenizer for consistency)
        neobert_inputs = textattack_tokenizer(
            example["sentence1"],
            example["sentence2"],
            truncation=True,
            max_length=128,
        ...
        print(neobert_inputs['input_ids']==textattack_inputs['input_ids']) #sanity check they are the same

        neobert_inputs['attention_mask'] = torch.where(neobert_inputs['attention_mask'] == 1, 0.0,-torch.inf).to(dtype=torch.float32) # NeoBERT expects float attention mask

        with torch.no_grad():
            neobert_outputs = neobert_stsb_model(**neobert_inputs)
            expert_usage_loss = neobert_outputs["expert_usage_loss"]
            if expert_usage_loss is not None:
                expert_usage_loss = expert_usage_loss.item()
            else:
                expert_usage_loss = 0.0
        
        # Store results
        textattack_preds.append(textattack_pred)
        neobert_expert_usage_losses.append(expert_usage_loss)
    ...
    results_df = pd.DataFrame({
        'sentence1': [example["sentence1"] for example in val_set],
        'sentence2': [example["sentence2"] for example in val_set],
