...
def download_directory(volume, remote_dir_str, local_dir, checkpoint: str = 'All', delete_in_modal: bool = False):
    ...
    if checkpoint != 'All':
        ...
        for entry in checkpoint_entries:
            ...
            if entry.type.name == "DIRECTORY":
                print(f"Creating local directory: {local_path}")
                local_path.mkdir(parents=True, exist_ok=True)
                download_directory(volume, remote_dir_str, local_dir, checkpoint_value, delete_in_modal)
            elif entry.type.name == "FILE":
                if local_path.exists():
                    print(f"Skipping already existing file: {local_path}")
                    continue
...
def download_runs_logs(base_path:Union[list[str],None] = None, checkpoints: Union[str,list[str]] =["1000,2000"], delete_in_modal: bool =False):
    ...
    if base_path == None:
        ...
        if checkpoints == "All":
            download_directory(runs_volume, remote_logs_dir_str, local_dir, delete_in_modal)
            ...
            for checkpoint in checkpoints:
                ...
                download_directory(runs_volume, remote_logs_dir_str, local_dir, checkpoint, delete_in_modal)
        ...
        for path in base_path:
            ...
            if checkpoints == 'All':
                download_directory(runs_volume, remote_logs_dir_str, local_dir, checkpoints, delete_in_modal)
                ...
                for checkpoint in checkpoints:
                    ...
                    try:
                        ...
                        print(f"error downloading checkpoint {checkpoint}: {e}. Moving to next checkpoint.")
                        continue
...
if __name__ == "__main__":
    ...
    if base_path is None:
        ...
        response = input("No base_path provided. Do you want to download all runs from Modal? (y/n): ")
        if response.lower() != 'n':
    ...
    with modal.enable_output(), app.run(detach=True):
        download_runs_logs(base_path=base_path, checkpoints=checkpoints, delete_in_modal=delete_in_modal)
